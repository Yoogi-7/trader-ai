...
def upgrade():
    ...
    # ---------- OHLCV (hypertable) ----------
    # PK MUSI zawierać kolumnę partycjonującą (ts) -> composite PK
    op.create_table(
        "ohlcv",
        sa.Column("symbol", sa.String(20), nullable=False),
        sa.Column("tf", sa.String(8), nullable=False),
        sa.Column("ts", sa.DateTime, nullable=False),  # UTC
        ...
        sa.PrimaryKeyConstraint("symbol", "tf", "ts", name="pk_ohlcv_symbol_tf_ts"),
    )
    op.create_index("idx_ohlcv_ts", "ohlcv", ["ts"])
    ...
    op.create_table(
        ...
        sa.Column("margin_mode", sa.String(10), nullable=False),  # isolated/cross
    ...
    op.create_table(
        ...
        sa.Column("rr", sa.Float, nullable=False),  # risk-reward
